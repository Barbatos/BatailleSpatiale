		...
		Scene(jeu)
{
	...
	new Bouton(&gui, Sauvegarder, "Sauvegarder la partie", x, 2 * y, largeur,
				hauteur);
	new Bouton(&gui, Charger, "Charger une partie", x, 3 * y, largeur, hauteur);
	new Bouton(&gui, Options, "Options", x, 4 * y, largeur, hauteur);
	new Bouton(&gui, QuitterPartie, "Quitter la partie", x, 5 * y, largeur,
				hauteur);
...
void SceneJeuMenu::surMessage(int id)
{
	switch (id)
	{
		case Retour:
