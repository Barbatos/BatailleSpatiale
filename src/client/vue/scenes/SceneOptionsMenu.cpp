...
SceneOptionsMenu::SceneOptionsMenu(Jeu& jeu) :
	...
	new Bouton(&gui, AugmenterMusique, "+", 1.25 * x, 1.3 * y / 2, largeur,
	           hauteur);
	new Bouton(&gui, BaisserMusique, "-", 0.75 * x, 1.3 * y / 2, largeur,
	           hauteur);
	new Bouton(&gui, Son, "Son", x, 2 * y / 2, largeur, hauteur);
	new Bouton(&gui, AugmenterSon, "+", 1.25 * x, 2.3 * y / 2, largeur,
	           hauteur);
	new Bouton(&gui, BaisserSon, "-", 0.75 * x, 2.3 * y / 2, largeur, hauteur);
	new Bouton(&gui, Retour, "Retour", x, 2 * y, largeur, hauteur);

	musique = jeu.lireRessources().lireMusique("oh_gg.ogg");
}
...
void SceneOptionsMenu::surMessage(int id) {
	...
	case AugmenterMusique:
		if (lireJeu().lireRessources().obtenirVolumeMusiques() < 100) {
			lireJeu().lireRessources().changerVolumeMusiques(
			    lireJeu().lireRessources().obtenirVolumeMusiques() + 1);
		}
		break;
	case BaisserMusique:
	...
	case AugmenterSon:
		if (lireJeu().lireRessources().obtenirVolumeSons() < 100) {
			lireJeu().lireRessources().changerVolumeSons(
			    lireJeu().lireRessources().obtenirVolumeSons() + 1);
		}
		break;
	case BaisserSon:
