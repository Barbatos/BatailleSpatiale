...
Animation::Animation(Gui* gui, int id, int x, int y, int largeur, int hauteur,
        ...
        Element(gui, id), sprite(
                gui->lireScene()->lireJeu().lireRessources().lireImage(
                        nomImage)), selection(), time(0), ajout(add), active(
                true), cyclique(cycle) {
    ecrirePosition(x, y);
...
void Animation::actualiser(float delta) {
    ...
    if (time > 1000 / 24) {
        if (active) {
            if ((selection.x * 480)
                    < (sprite.getTexture()->getSize().x - 480)) {
                selection.x++;
            }
            else {
                if ((selection.y * 480)
                        < (sprite.getTexture()->getSize().y - 480)) {
                    selection.y++;
                }
                else {
                    selection.y = 0;
            ...
            if (!cyclique) {
                if ((selection.x + 1 == sprite.getTexture()->getSize().x / 480)
                        and (selection.y + 1
                                == sprite.getTexture()->getSize().y / 480)) {
                    active = false;
                }
