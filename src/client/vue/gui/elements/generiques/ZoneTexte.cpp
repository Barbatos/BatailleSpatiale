...
ZoneTexte::ZoneTexte(Gui* gui, int id, int x, int y, int largeur, int hauteur,
    ...
    label.setFont(gui->lireScene()->lireJeu().lireRessources().lirePolice("grand9k.ttf"));
}
...
void ZoneTexte::entreeTexte(sf::Uint32 unicode) {
    ...
    if (selectionne) {
        ...
        if (estAutorise(unicode) && label.getGlobalBounds().width < rectangle.getSize().x - 20) {
            ...
            curseur.setPosition(label.getPosition().x + label.getGlobalBounds().width + 1, curseur.getPosition().y);
        }
        // Si le caractère entré est la touche de retour arrière
            ...
            if (texte.size() == 0)
                curseur.setPosition(rectangle.getPosition().x + 5, curseur.getPosition().y);
            else
                curseur.setPosition(label.getPosition().x + label.getGlobalBounds().width + 1, curseur.getPosition().y);
        }
