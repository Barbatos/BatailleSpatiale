...
AffichageCase::AffichageCase(Gui* gui, int id, float x, float y, float taille,
		...
		Element(gui, id), animation(nullptr), image(), fond(), position(
				position), selectionne(false), gui(gui)
{
	ecrirePosition(x, y);
...
void AffichageCase::actualiser(float)
{
	...
	{
		if (animation != nullptr && animation->lireFinie())
		{
			delete animation;
			animation = nullptr;
		}
